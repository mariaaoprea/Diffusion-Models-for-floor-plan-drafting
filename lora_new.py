...
def main():
    ...
    vae_lora_config = LoraConfig(
        ...
        lora_alpha=args.rank,
        init_lora_weights="gaussian",
        target_modules=["encoder.conv1", "encoder.conv2", "decoder.conv1", "decoder.conv2"]
    )
    text_encoder_lora_config = LoraConfig(
        r=args.rank,
        lora_alpha=args.rank,
        init_lora_weights="gaussian",
        target_modules=["embeddings", "encoder.layer.0.attention.self.query"]
    )
    ...
    vae_lora_layers = filter(lambda p: p.requires_grad, vae.parameters())
    text_encoder_lora_layers = filter(lambda p: p.requires_grad, text_encoder.parameters())

    # Combine all the layers that require gradients
    lora_layers = list(unet_lora_layers) + list(vae_lora_layers) + list(text_encoder_lora_layers)
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt is not None:
            ...
            # generator is created and used to generate images based on the seed.
            generator = torch.Generator(device=accelerator.device)
            if args.seed is not None:
                generator = generator.manual_seed(args.seed)
